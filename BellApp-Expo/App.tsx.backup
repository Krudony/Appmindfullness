/**
 * BellApp - แอปแจ้งเตือนระฆัง (Expo Go Version)
 * QR Code Development Workflow - WORKING VERSION!
 */
...
function App() {
  ...
  const [currentIP, setCurrentIP] = useState('192.168.2.48:8081');
  ...
  const updateDevelopmentURL = () => {
    // Get development server URL (Expo automatically handles this)
  ...
  const playBellSound = async () => {
    try {
      ...
      const { sound } = await Audio.Sound.createAsync(
        require('./assets/bell.mp3'), // You'll need to add this sound file
        { shouldPlay: true }
      );
      setSound(sound);
    } catch (error) {
  ...
  const handleStartService = async () => {
    if (!isServiceRunning) {
      try {
        ...
        setIsServiceRunning(true); // Still allow basic functionality
      }
    } else {
  ...
  const openDevelopmentURL = () => {
    Linking.openURL(`http://${currentIP}`);
  };
      ...
      <View style={styles.content}>
        <View style={styles.statusCard}>
          ...
          <View style={styles.statusRow}>
            ...
            <TouchableOpacity onPress={openDevelopmentURL}>
